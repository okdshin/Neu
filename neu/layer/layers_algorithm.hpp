...
namespace neu {
	...
	*/
	...
	/*
	decltype(auto) layers_test_forward(std::vector<layer::any_layer>& layers,
	...
	decltype(auto) layers_forward(std::vector<layer::any_layer>& layers,
			gpu_vector const& initial_input, gpu_vector& output,
			boost::compute::command_queue& queue
				=boost::compute::system::default_queue()) {
		gpu_vector input(initial_input.begin(), initial_input.end(), queue);
	...
	decltype(auto) layers_backward(std::vector<layer::any_layer>& layers,
	...
	// TODO update async
	decltype(auto) layers_update(std::vector<layer::any_layer>& layers,
			boost::compute::command_queue& queue
				=boost::compute::system::default_queue()) {
		for(auto& l : layers) {
			if(l.should_update()) {
