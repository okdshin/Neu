#ifndef NEU_LAYER_IMPL_MATRIX_MULTIPLY_HPP
#define NEU_LAYER_IMPL_MATRIX_MULTIPLY_HPP
//20160228
...
namespace neu {
	namespace layer {
		namespace impl {
			constexpr char matrix_multiply_kernel_source[] = BOOST_COMPUTE_STRINGIZE_SOURCE(
					...
					const int m_size, const int n_size, const int k_size)
				{
					...
					for (int k = 0; k < k_size; ++k) {
						acc += a[m*k_size + k + a_offset] * b[k*n_size + n + b_offset];
					}
					c[m*n_size + n + c_offset] = acc;
				}
			...
			decltype(auto) matrix_multiply(
				...
				static auto mm_kernel = neu::make_kernel(
					neu::layer::impl::matrix_multiply_kernel_source,
...
#endif //NEU_LAYER_IMPL_MATRIX_MULTIPLY_HPP
