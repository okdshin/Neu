#ifndef NEU_CONVOLUTION_OPTIMIZED_WR_LAYER_KERNEL_SOURCE_HPP
#define NEU_CONVOLUTION_OPTIMIZED_WR_LAYER_KERNEL_SOURCE_HPP
//20151005
#include <boost/compute/kernel.hpp>
namespace neu {
	namespace layer { 
		constexpr char convolution_optimized_wr_kernel_source[] =
			...
			__kernel void update_tile64_wpt2_reg(
				const __global float* delta,
				const __global float* input,
				__global float* del_filters,
				const int m_size, const int n_size, const int k_size,
						...
						for(int l = 0; l < lpt; ++l) {
							...
							a_sub[row][col] =
								offsetm+row < m_size && tile_size*t+col < k_size ?
									delta[bat*output_channel_num*output_width*output_width+
										(offsetm+row)*k_size+tile_size*t+col] : 0.f;

							const int tiled_row = tile_size*t+row;
							const int global_col = offsetn+col;
							if(tiled_row < k_size && global_col < n_size) {
								const int k = global_col/(filter_width*filter_width);
								const int ff = global_col%(filter_width*filter_width);
								const int fr = ff/filter_width;
				...
				for(int wm = 0; wm < works_per_thread; ++wm) {
					...
					for(int wn = 0; wn < works_per_thread; ++wn) {
						...
						if(global_col < n_size && global_row < m_size) {
							del_filters[global_row*n_size+global_col] = sum[wm][wn]/batch_size;
						}
					}
				}
			}
		...
		decltype(auto) convolution_optimized_wr_forward(
				neu::kernel& ker,
		...
		decltype(auto) convolution_optimized_wr_update_tile32(
				neu::kernel& ker,
			...
			queue.enqueue_nd_range_kernel(ker, 2, nullptr, global, local);
		}
		template<typename InputRange>
		decltype(auto) convolution_optimized_wr_update_tile64_wpt2_reg(
				neu::kernel& ker,
				InputRange const& delta, gpu_vector const& input, gpu_vector& del_filters,
				int batch_size,
				int input_width, int filter_width, int output_width,
		...
		template<typename InputRange, typename OutputRange>
		decltype(auto) convolution_optimized_wr_backward(
				neu::kernel& ker,
...
#endif //NEU_CONVOLUTION_OPTIMIZED_WR_LAYER_KERNEL_SOURCE_HPP
